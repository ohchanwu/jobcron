...
`job-scraper`는 로컬 웹 앱을 여는 단일 바이너리(binary)입니다. **스크랩 시작**을
...
job-scraper는 *감정적 측면(emotional layer)*을 위해 만들어졌습니다: 매일의 고된 작업을
...
- **중요한 것을 가까이 둡니다** — 지금까지 스크랩한 모든 것을 담은 관심 공고 보관함,
...
- **선택적 AI 점수화 (당신의 키를 직접 사용).** Anthropic 또는 OpenAI 키를 추가하면
...
[최신 릴리스](https://github.com/ohchanwu/job-scraper/releases/latest)에서 사용하는
...
**macOS (Intel)** — `job-scraper_darwin_amd64.tar.gz`를 사용하세요.
...
**Windows** — `job-scraper_windows_amd64.zip`를 내려받아 압축을 풀고
`job-scraper.exe`를 실행하세요.

### 첫 실행 시 참고 사항 (First-run notes)

- **macOS Gatekeeper**가 서명되지 않은 바이너리를 차단할 수 있습니다. 파일을 우클릭 →
  Open을 선택하거나, `xattr -d com.apple.quarantine ./job-scraper`를 실행하세요.
...
[data.go.kr](https://www.data.go.kr)에서 발급하는 무료 키이며, `JOBSCRAPER_WORKNET_KEY`
환경 변수에서도 읽습니다), `--version`.

당신의 데이터는 OS 설정 디렉터리 아래 하나의 SQLite 파일에 저장됩니다
(macOS는 `~/Library/Application Support/job-scraper/`, Linux는 `~/.config/job-scraper/`,
Windows는 `%APPDATA%\job-scraper\`).
...
- **브리핑은 오늘 올라온 공고입니다.** 첫 화면은 오늘 처음 발견된 것을 보여줍니다 — 그것이
  일일 의식입니다. 지금까지 스크랩한 모든 것은 관심 공고에 남아 있으므로(날짜순 또는
...
기본적으로 꺼져 있습니다. 프로필 폼에서 **AI 분석 (선택)**을 열고, 제공자(Anthropic 또는
OpenAI)를 선택하고, 당신의 API 키를 붙여넣은 뒤, 몇 개의 자유 서술형 목표(어떤 일을
...
이것은 **v2.0** 라인이며, 실제 제공자 경로가 현실 세계에서 더 많은 검증을 거치는 동안
