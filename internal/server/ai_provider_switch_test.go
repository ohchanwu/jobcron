...
func TestProviderFailureMessageClassifies(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "Gemini quota failure with retry info", err: &ai.APIError{Status: http.StatusTooManyRequests, Body: `{"error":{"code":429,"message":"You exceeded your current quota, please check your plan and billing details. ` + privateMarker + `","status":"RESOURCE_EXHAUSTED","details":[{"@type":"type.googleapis.com/google.rpc.QuotaFailure","violations":[{"quotaMetric":"generativelanguage.googleapis.com/generate_content_free_tier_requests","quotaId":"GenerateRequestsPerDayPerProjectPerModel-FreeTier","quotaDimensions":{"model":"gemini-3.5-flash-lite","location":"global"}}]},{"@type":"type.googleapis.com/google.rpc.RetryInfo","retryDelay":"43s"}]}}`}, want: []string{"사용 한도를 초과", "사용량·할당량"}, reject: []string{"잠시 후 다시", "계속되면"}},
